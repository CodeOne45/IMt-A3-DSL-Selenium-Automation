...
{
    String xpath = "//a[contains(text(), 'Toutes les actualités')]";
    Predicate<WebElement> predicate = (elt) -> {
                            boolean filter = true;
                            filter &= elt.isDisplayed();
                            return filter;
                        };
...
{
    String xpath = "//a[contains(text(), 'Accueil')]";
    Predicate<WebElement> predicate = (elt) -> {
                            boolean filter = true;
                            filter &= elt.isDisplayed();
                            return filter;
...
{
    String xpath = "//a[text()='Toutes les actualités']";
    List<WebElement> current = findElementsStable(driver, By.xpath(xpath), 10, 5);
    if (current.isEmpty()) {
        throw new WebDriverException("could not find any element matching xpath " + xpath);
    }
