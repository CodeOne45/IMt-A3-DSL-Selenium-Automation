...
public class Test_8 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        {
            String dataToSave = "";
            String xpath = "//div[contains(@class, 'slick-list')]//div[contains(@class, 'slick-slide')]//div[contains(@class, 'carrou_link')]//a";
            ...
            if (index == -1) {
                for (WebElement e : current) {
                    dataToSave += e.getDomAttribute("href").toString();
                    savedData.put("post url", dataToSave);
                }    ;
            } else {
        ...
        {
            String dataToSave = "";
            String xpath = "//div[contains(@class, 'slick-list')]//div[contains(@class, 'slick-slide')]//div[contains(@class, 'carrou_tit')]//h2";
            List<WebElement> current = findElementsStable(driver, By.xpath(xpath), 10, 5);
            if (current.isEmpty()) {
                throw new WebDriverException("could not find any element matching xpath " + xpath);
            }
            ...
            if (index == -1) {
                for (WebElement e : current) {
                    ...
                    savedData.put("post title", dataToSave);
                }    ;
            } else {
                if (index < current.size()) {
                    WebElement e  = current.get(index);
        ...
        driver.get("http://www.imt-atlantique.fr/fr/rechercher");
        {
            ...
            if (index == -1) {
                for (WebElement e : current) {
                    e.sendKeys(""+getSavedData("post title")+"");
                }    ;
            } else {
                if (index < current.size()) {
                    WebElement e  = current.get(index);
        ...
        {
            String xpath = "//*[self::ul or self::ol][@id='ui-id-2']//li";
            List<WebElement> current = findElementsStable(driver, By.xpath(xpath), 10, 5);
            if (current.isEmpty()) {
                throw new WebDriverException("could not find any element matching xpath " + xpath);
            }
        ...
        {
            String xpath = "//*[contains(@href, '"+getSavedData("post url")+"')]";
            List<WebElement> current = findElementsStable(driver, By.xpath(xpath), 10, 5);
            if (current.isEmpty()) {
                throw new WebDriverException("could not find any element matching xpath " + xpath);
            }
